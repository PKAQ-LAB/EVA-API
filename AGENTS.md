...
你熟悉 java、spring boot 、xxl-job 、mybatis-plus、mongodb、redis、mysql等，擅长编写 java 项目且高性能的代码。
所有的修改都在cornerstone分支上直接进行
...
This project is indexed by GitNexus as **EVA-API** (3851 symbols, 8734 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST run `detect_changes()` before committing** to verify your changes only affect expected symbols and execution flows. For regression review, compare against the default branch: `detect_changes({scope: "compare", base_ref: "cornerstone"})`.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/EVA-API/context` | Codebase overview, check index freshness |
| `gitnexus://repo/EVA-API/clusters` | All functional areas |
| `gitnexus://repo/EVA-API/processes` | All execution flows |
| `gitnexus://repo/EVA-API/process/{name}` | Step-by-step execution trace |
