package tech.yunyue.core.cache.util;
...
public class RedisUtil {
    ...
    public Map<String, ?> getAll(String key) {
        ...
        keys.stream().forEach(item -> {
            map.put(item + "", JsonUtil.parse(this.get(item + ""), Map.class));
        });
     ...
     * @return
     */
    public Map<String, ?> getPureAll(String key) {
        Set<Object> keys = redisTemplate.keys(key + "*");
        Map<String, Object> map = new LinkedHashMap<>(keys.size());

        int subIndex = key.length() + 2;
        ...
        keys.stream().forEach(item -> {
            ...
            if (null != value && value instanceof String) {
                map.put(k, JsonUtil.parse(String.valueOf(value), Map.class));
            } else {
                map.put(k, value);
            }
        });
     ...
     * @param prex
     */
    public void remove(String prex) {
