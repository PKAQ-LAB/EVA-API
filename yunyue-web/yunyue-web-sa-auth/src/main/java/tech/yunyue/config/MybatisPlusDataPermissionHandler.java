...
public class MybatisPlusDataPermissionHandler implements DataPermissionHandler {
    ...
    public Expression getSqlSegment(Expression where, String mappedStatementId) {
        ...
        // 根据权限拼接查询语句
        if (CharSequenceUtil.isNotBlank(permissionSQL) && !ObjectUtil.equal(permissionSQL, "true")) {
    ...
    public static String permissionSql(List<ThreadUser.GrantedRoles> dataPermission, String tableName) {
        ...
        dataPermission.stream().anyMatch(item -> {
            ...
            String deptOrPostIds = item.getDataPermissionDeptid();
            String sql = buildSqlForPermission(tableName, depId, postId, uId, deptOrPostIds, permissionEnum);
