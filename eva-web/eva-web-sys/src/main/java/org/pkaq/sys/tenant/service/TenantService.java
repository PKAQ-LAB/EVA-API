...
//     * 新增/编辑一条租户信息
//     */
//    @BizLog(operateType = BizLogEnum.CREATE_UPDATE, description = "保存租户[{0}]", args = {"param:0.id"})
//    @Transactional
...
//        editBo.setId(tid);
//
//        TenantEntity entity = BeanUtil.copyProperties(editBo, TenantEntity.class);
//        if (isNew) {
//            var adminId = IdWorker.getIdStr();
//            entity.setAdminId(adminId);
//            entity.setStatus(LockEnumm.UNLOCK.getCode());
...
//        var vo = BeanUtil.copyProperties(entity, TenantDetailVo.class);
//        // 查询管理员账号
...
//        IPage<TenantListVo> pv = this.mapper.listPage(pagination, queryBo);
//        BeanUtils.copyProperties(pagination, pv);
//        return pv;
//    }
//
//    /**
...
//        LockEnumm lockEnumm = Arrays.stream(LockEnumm.values()).filter(e -> e.getCode().equals(bo.getStatus())).findFirst().orElse(null);
//        if (Objects.isNull(lockEnumm)) return;
//        this.mapper.updateById(new TenantEntity().setStatus(bo.getStatus()).setId(bo.getId()));
//    }
//
//    /**
//     * 校验code/名称是否唯一
...
//        user.setTenantCode(editBo.getCode());
...
//                // 删除redis中的角色缓存
//                roleService.evictUserRole(tenant.getAdminId());
...
//        var list = this.mapper.selectList(Wrappers.<TenantEntity>lambdaQuery().eq(TenantEntity::getStatus, LockEnumm.UNLOCK.getCode()));
